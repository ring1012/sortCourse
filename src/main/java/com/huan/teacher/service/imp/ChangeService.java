...
public class ChangeService {
	...
	public void excute(ChangeForm cf,startSortCourse myCourse,HttpServletRequest request) throws Myexception{
		int lessonNum=myCourse.lessonNum;
		int classNum=myCourse.classNum;
		int table[][]=new int [classNum][lessonNum*7];
		int count=0;
		int fixt[]=cf.fixTable;
		for(int i=0;i<classNum;i++){
			for(int j=0;j<lessonNum;j++){
				table[i][j]=fixt[count++];
			}
		}
...
//		}
		SA sa=myCourse.sa;
		sa.changeAttr(table,cf.changeStr,sa.sheetInfor);
